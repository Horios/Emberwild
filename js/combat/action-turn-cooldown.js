...
GAME_BALANCE_DEFAULTS.combat.minimumActiveCooldown=1;
function migrateActiveCooldownFloorDocument(input){
  if(!input||typeof input!=='object')return input;
  const out=JSON.parse(JSON.stringify(input));
  out.balanceSettings??={};out.balanceSettings.meta??={};
  if(Number(out.balanceSettings.meta.activeCooldownFloorVersion||0)<1){
    if(Number(out.balance?.combat?.minimumActiveCooldown)===2)out.balance.combat.minimumActiveCooldown=1;
    out.balanceSettings.meta.activeCooldownFloorVersion=1;
  }
  return out;
}
let actionTurnMigratedSaved=null;
try{
  const raw=localStorage.getItem(BALANCE_KEY);
  if(raw){
    const parsed=JSON.parse(raw),migrated=migrateActiveCooldownFloorDocument(parsed);
    actionTurnMigratedSaved=migrated;
    if(JSON.stringify(parsed)!==JSON.stringify(migrated))localStorage.setItem(BALANCE_KEY,JSON.stringify(migrated));
  }
}catch(e){console.warn('冷卻下限設定 migration 失敗',e);}
GAME_BALANCE=normalizeRuntimeBalance(actionTurnMigratedSaved?.balance||GAME_BALANCE);
const actionTurnApplyBalanceConfigBase=applyBalanceConfig;
applyBalanceConfig=function(input,options){return actionTurnApplyBalanceConfigBase(migrateActiveCooldownFloorDocument(input),options);};
const actionTurnBalanceReferenceBase=balanceReference;
balanceReference=function(){const r=actionTurnBalanceReferenceBase();r.affixTypes['16']='全主動／輔助技能冷卻減少（回合）';r.balancePaths['balance.affixes.extendedValues.allCooldownReduction']='新全技能冷卻詞條；只影響有冷卻的主動與輔助技能，單位為角色回合';r.units.cooldown='回合；冷卻 N 表示施放後需經過 N 次該角色自身行動，之後才能再次施放';return r;};
...
globalThis.performEnemyActionCore ??= (e=>performEnemyBasic(e));
function performEnemyAction(e){return globalThis.performEnemyActionCore(e);}
pacedRound=function*(){if(!party||!running||$('modal').open)return;currentEvents=[];lastActionOrder=[];actionVisualTime=Date.now();partyClock++;round++;effects=effects.filter(e=>e.until>partyClock);if(!foes.some(e=>e.hp>0)){spawnGroup();if(tab==='battle'||!isEditingControl())render();else refreshGlobalJournal();return;}for(const h of living()){const v=battleStats(h),regen=effectTotal(heroKey(h),'regen'),beforeRegen=h.hp;h.hp=Math.min(v.hp,h.hp+v.hp*regen);const regenHealed=Math.max(0,h.hp-beforeRegen);if(regenHealed>0)recordRegenContribution(h,regenHealed);if(h.autoPotion&&h.hp<v.hp*GS('combat.autoPotionThreshold',.35)&&h.potions>0)withHero(h,()=>potion(true));}const order=initiativeOrder();for(const unit of order){if(!living().length||!foes.some(e=>e.hp>0))break;if(unit.actor.hp<=0)continue;if(unit.side==='hero'){const h=unit.actor,readySupport=h.supportSlots.filter(i=>i!==null&&cooldownLeft(supportCooldowns,h.job+'-'+i)<=0),readyActive=h.active.filter(i=>i!==null&&cooldownLeft(actorCooldowns,h.job+'-'+i)<=0);tickHeroCooldowns(h);for(const i of readySupport){const key=h.job+'-'+i;if(castSupport(h,i)!==false){recordAction(heroKey(h),characterName(h),'輔助');supportCooldowns[key]=supportSkillCooldown(h,i);yield;if(!living().length||!foes.some(e=>e.hp>0))break;}}let actionTaken=false;for(const i of readyActive){if(!foes.some(e=>e.hp>0))break;const key=h.job+'-'+i;if(castPartySkill(h,i,battleStats(h))!==false){actorCooldowns[key]=Math.max(0,Math.round(skillCooldown(i,h)));recordAction(heroKey(h),characterName(h),'技能');actionTaken=true;yield;break;}}if(actionTaken)continue;if(singleTargetEnemyPool().length){recordAction(unit.id,characterName(h),'普攻');performHeroBasic(h);yield;}}else{const label=typeof sharedEnemyActionLabel==='function'?sharedEnemyActionLabel(unit.actor):'攻擊';recordAction(unit.id,unit.actor.name,label);performEnemyAction(unit.actor);yield;}}for(const e of foes)if(e.hp<=0)rewardGroupKill(e);if(!living().length){running=false;party.members[0].gold-=Math.floor(party.members[0].gold*GS('combat.defeatGoldLoss',.05));for(const h of heroes())h.shield=0;note('全隊戰敗，帳號失去 '+Math.round(GS('combat.defeatGoldLoss',.05)*10000)/100+'% 金幣。生命已全滿。');refillParty();foes=[];enemy=null;}else if(!foes.some(e=>e.hp>0)){refillParty();note('本次遭遇勝利，全隊生命已回滿。');if(foes.some(e=>e.kind==='final')){running=false;note('噬日者已倒下！LV30 成員解鎖 LV60，其他成員達 LV30 時解鎖。');}}save();if(tab==='battle'||!isEditingControl())render();else refreshGlobalJournal();if(activeBattleStat){if(activeBattleStat.pendingOutcome==='wipe')finalizeBattleStatistics('wipe');else if(foes.length&&foes.every(e=>e.hp<=0))finalizeBattleStatistics('victory');}};
...
applyGameplaySettingsSideEffects=function(){actionTurnApplyGameplaySettingsBase();GEMS[1].desc='主動技能冷卻 −'+GS('skills.gems.cooldownReduction',1)+' 回合（最低 '+GAME_BALANCE.combat.minimumActiveCooldown+'）／觸發率 +'+Math.round(GS('skills.gems.cooldownProcBonus',.10)*10000)/100+'%';};
applyGameplaySettingsSideEffects();
const actionTurnGuideBase=guideView;
guideView=function(){return `<section class="panel"><h2>行動回合冷卻</h2><p>主動與輔助技能的冷卻只在該角色自己的行動回合推進；冷卻 N 代表施放後要完整經過 N 次自身行動，之後才能再次施放。其他隊員與敵人行動不會扣除冷卻，戰鬥倍速也只改變播放速度。</p><p>每個角色自己的行動中，輔助技能不消耗主要行動；輔助處理完後，最多只會施放 1 個可用主動技能，若沒有主動技能可施放才會普通攻擊。主動技能或普通攻擊一旦執行，就結束該角色本回合。</p><p>主動技能最低冷卻為 ${GAME_BALANCE.combat.minimumActiveCooldown} 回合；因此 1 回合冷卻時，至少會隔一個自己的行動回合才再次施放。裝備的全技能冷卻詞條與指定技能冷卻詞條可同時生效。</p></section>`+actionTurnGuideBase();};
try{GAMEPLAY_SETTINGS=normalizeGameplaySettings(GAMEPLAY_SETTINGS);globalThis.__EMBERWILD_GAMEPLAY_SETTINGS=GAMEPLAY_SETTINGS;applyGameplaySettingsSideEffects();}catch(e){console.warn('行動回合設定初始化失敗',e);}
