...
function skillLoadoutSummaryHTML(){
  ...
  const group=(label,slots,nameOf,clearFn)=>`<div class="skill-loadout-group"><b>${label}</b><div class="skill-loadout-slots">${slots.map((id,slot)=>`<span class="skill-loadout-slot"><span>槽 ${slot+1}</span><strong>${esc(nameOf(id))}</strong><button onclick="${clearFn}(${slot})" ${id===null?'disabled':''}>清空</button></span>`).join('')}</div></div>`;
  return `<section class="skill-loadout-panel">${group('主動技能',active,coreName,'clearActiveSkill')}${group('觸發技能',proc,coreName,'clearProcSkill')}${group('輔助技能',support,supportName,'slotSupport.bind(null,null)')}</section>`.replace(/slotSupport\.bind\(null,null\)\((\d+)\)/g,'slotSupport(null,$1)');
}
