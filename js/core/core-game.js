...
function wornEquipmentView(){return heading('PARTY EQUIPMENT / 穿戴總覽','已穿戴裝備',`<span class="tag">${party.members.length} 位角色</span>`)+`<div class="worn-overview">${party.members.map(h=>`<section class="panel worn-member"><div class="row"><h2>${esc(characterName(h))}</h2><span class="small">${CLASSES[h.job].name} · LV ${h.lv} · ${party.active.includes(h.job)?'出戰':'候補'}</span></div>${[0,1,2,3,4].map(position=>{const g=h.bag.find(x=>x.id===h.equipped[position]);return `<article class="worn-slot"><div class="worn-slot-head">${g?`<b>${equipmentNameHTML(g)}</b><button onclick="heroMenuAction(${h.job},()=>openForge('${g.id}'))">強化／洗鍊</button>`:`<b class="slot-label">${EQUIP_POSITION_NAMES[position]}</b><span class="small">未穿戴</span>`}</div>${g?`<p class="small equipment-total-summary">${globalThis.equipmentTotalSummaryText(g)}</p>${globalThis.equipmentAttributeDetailsHTML(g)}`:''}</article>`;}).join('')}</section>`).join('')}</div>`;}
