...
$('app').innerHTML=`<div class="layout"><aside><div class="eyebrow">YOUR ADVENTURER</div><div class="portrait">${pixel(state.job)}</div><h2>${esc(characterName(state))}</h2><div class="row"><span class="level">LV. ${String(state.lv).padStart(2,'0')}</span><span class="small">${state.lv===levelCap(state)?'等級已達上限':`${state.xp} / ${need(state.lv)} EXP`}</span></div>${meter(state.lv===levelCap(state)?1:state.xp,state.lv===levelCap(state)?1:need(state.lv),'gold')}<nav>${[['battle','','荒野探索'],['roster','','隊伍編成'],['character','','角色能力'],['skills','','技能'],['equipment','▣','背包'],['worn','','已穿戴裝備'],['forge','','裝備強化'],['bossCraft','','BOSS 製作'],['quests','▤','委託手札'],['shop','◈','商店'],['textRarity','','文字稀有度設計器'],['guide','','冒險指南']].map(([id,icon,name])=>`<button class="${tab===id?'active':''}" onclick="setTab('${id}')">${name}</button>`).join('')}</nav><div class="aside-foot">遠征目標<br><span class="gold">${state.won?' 已突破 · 上限 LV60':'LV30 · 挑戰噬日者'}</span><div class="divider"></div>累計擊敗 ${state.totalKills} 隻<br>自動儲存 · 每 5 秒</div></aside><main>${tutorial()}${({battle:battleView,roster:rosterView,character:characterView,skills:skillsView,equipment:equipmentView,worn:wornEquipmentView,forge:forgeView,bossCraft:bossCraftView,quests:questView,shop:shopView,textRarity:globalThis.textRarityView||guideView,guide:guideView}[tab])()}</main>${sharedBattleJournal()}</div>`;}
...
guideView=function(){return `<section class="panel"><h2>3.0 · 小隊遠征</h2><p>可招募四職業各一位，最多三人出戰。角色操作頁可獨立切換對象；金幣、材料、寶石、消耗品與背包由帳號共用，角色保有各自的穿戴及技能，任務由帳號共用。招募者與出戰最低等級一致，需自行配置能力點、技能、二轉及裝備。</p><p>固定群怪模式，每場3～6隻。終焉王座群怪模式為噬日者及最多五隻星隕護衛。每隻怪物只結算一次戰利品，隨機交給一位出戰角色；經驗由出戰成員平分，倒下成員也分得經驗，候補不分得。治療技能自動支援生命比例最低的存活隊員，吸血僅回復本人且不計溢出傷害。</p><p>每人有兩個主動槽、兩個輔助槽，以及已配置的普攻觸發技。輔助技能不消耗主要行動；輔助處理後，每名角色在自己的回合最多只會施放 1 個可用主動技能，若沒有可用主動技能才進行普攻與觸發判定。主動技能或普攻執行後即結束該角色回合。速度只決定行動先後，不增加每回合行動次數。BUFF／DEBUFF 以戰鬥回合計時，暫停和對話框不會推進；換圖、換出戰成員會清除遭遇及效果。全隊戰後回滿生命，倒下隊員可在戰後恢復。</p><p>多來源減傷最高 75%、破甲最高 75%、敵方攻擊降低最高 70%、戰鬥暴擊率最高 85%。存檔保存全隊養成、未確認洗鍊與消耗品剩餘回合；不保存當前敵人與戰鬥技能效果。</p></section>`+solo.guideView();};
...
function equipmentNameHTML(g){const type=SLOTS[g?.slot]||'裝備';return `<span class="enhanced-name ${equipmentNameClass(g)}" title="特效品質：${equipmentEffectRank(g)<0?'無':AFFIX_RANK[equipmentEffectRank(g)]}；強化 +${g.plus||0}">${esc(type+' '+equipmentDisplayName(g))}</span>`;}
...
function unequipSharedGear(id){ensureSharedGear();const wearer=gearWearer(id);if(!wearer)return;wearer.equipped=wearer.equipped.map(x=>x===id?null:x);withHero(wearer,clampVitals);save();render();}
...
skillsView=function(){return singlePagePanel('skills','技能 <span class="skill-rules-help" tabindex="0" aria-label="技能規則說明" data-tooltip="1. 主動技能／輔助技能的冷卻只在該角色自己的回合推進；冷卻 N 代表施放後要完整經過 N 次自身行動，之後才能再次施放。主動技能最低可降至 1 回合。&#10;2. 輔助技能不消耗主要行動；輔助處理後，本回合只會執行 1 個主動技能或 1 次普攻。主動技能與普攻執行後都會立即結束該角色回合。&#10;3. 主動技能不會觸發普攻觸發技能；觸發技能只會在普攻後判定。">?</span>',()=>`<div class="actions skill-subtabs"><button class="${skillPanel==='active'?'primary':''}" onclick="setSkillPanel('active')">主動技能</button><button class="${skillPanel==='proc'?'primary':''}" onclick="setSkillPanel('proc')">普攻觸發技能</button><button class="${skillPanel==='support'?'primary':''}" onclick="setSkillPanel('support')">輔助技能</button></div>${skillPanel==='active'?coreSkillRows():skillPanel==='proc'?procSkillRows():supportSkillRows()}`);};
...
async function importBalanceJSON(ev){const f=ev.target.files[0];ev.target.value='';if(!f)return false;try{if(f.size>4e6)throw Error('檔案過大');const data=JSON.parse(await f.text());applyBalanceConfig(data);toast('測試設定已套用；重新開啟仍會保留');return true;}catch(e){toast('測試設定匯入失敗：'+e.message);return false;}}
...
function prepareBalanceForBootstrap(input){
  ...
  // The reroll item is installed by a late equipment module, after this pass.
  if(Array.isArray(copy.items))copy.items=copy.items.filter(item=>item?.id!=='power_tier_reroll');
  return copy;
}
try{
